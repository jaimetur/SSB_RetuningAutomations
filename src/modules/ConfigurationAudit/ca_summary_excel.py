...
def build_summary_audit(
    ...
    tips_by_metric: Dict[tuple[str, str], str] = {
        ...
        ("NRFreqRelation", f"NR nodes with the old N77 SSB ({n77_ssb_pre}) but without the new N77 SSB ({n77_ssb_post})"): " ",
        ...
        ("GUtranFreqRelation", f"LTE nodes with GUtranFreqRelationId {expected_old_rel_id} and {expected_new_rel_id}"): " ",
        ("GUtranFreqRelation", f"LTE nodes with GUtranFreqRelationId {expected_old_rel_id} but without {expected_new_rel_id}"): " ",
        ("GUtranFreqRelation", f"LTE nodes with the N77 SSB not in ({n77_ssb_pre}, {n77_ssb_post})"): " ",
        ("GUtranFreqRelation", f"LTE nodes with Auto-created GUtranFreqRelationId to new N77 SSB ({n77_ssb_post}) but not following VZ naming convention ({n77_ssb_post}-30-20-0-1)"): " ",
